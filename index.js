...
const filesToInit = ["business.json", "chats.json", "payments.json", "contacts.json", "broadcasts.json"];
for (const f of filesToInit) {
  const file = path.join(BOT_DATA_DIR, f);
  if (!fs.existsSync(file)) {
    fs.writeFileSync(file, f === "business.json" ? "{}" : "[]");
  }
}
...
// API broadcast (JSON response)
app.post('/send-broadcast', async (req, res) => {
  const { message, numbers } = req.body;
  if (!message) return res.status(400).json({ error: 'message required' });
  if (!isClientReady) return res.status(503).json({ error: 'whatsapp client not ready' });

  const targets = Array.isArray(numbers) && numbers.length
    ? numbers
    : loadJson(path.join(BOT_DATA_DIR, 'contacts.json'), []).map(c => c.number || c.phone).filter(Boolean);
